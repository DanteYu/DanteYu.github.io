...
####. a
#### .any
`.any`使得所有`.keys`验证只要求期待对象包含至少一个指定的keys。与`.all`的作用相反。`  expect({a: 1, b: 2}).to.have.any.keys('a', 'd');`
#### .all
`.all`使得所有`.keys`验证只要求期待对象包含所有指定的keys。与`.any`的作用相反。`    expect({a: 1, b: 2}).to.have.any.keys('a', 'd');`
