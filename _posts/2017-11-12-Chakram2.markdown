...
#### HTTP PUT
使用`chakram.put(url, data, params)`完成一次HTTP GET请求。此方法返回的promise对象最终会被resolve成一个`ChakramResponse`对象。
...
describe("Check Wolf Warrir 2", function(){
  it("should check array in body", function () {
      var response = chakram.post("http://httpbin.org/post", ["nba", "spurs"]);
      expect(response).to.have.json("json", ["nba", "spurs"]);
      expect(response).to.have.header('content-type', "application/json");
      expect(response).to.have.json('headers.Host', 'httpbin.org');
      return chakram.wait();
  });
})
```
