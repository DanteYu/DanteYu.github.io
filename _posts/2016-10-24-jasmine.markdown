...
使用这个命令会创建 ./spec/support这样一个目录，然后会在support下有jasmine.json这样一个配置文件
...
jasmine的全局方法 describe() 代表着测试套件 test suite的实现。

describe(string, function)方法有两个参数
- string指整个测试套件的名字或是title
- function指整个测试套件的实现代码

jasmine的全局方法 it() 代表着测试用例/测试规格 test specs的实现。
...
在jasmine中，expectation是用方法expect(actual_value)实现的. 给expect传入的应该是系统实际运行出来的值。expect方法与Matcher方法链接，而Matcher方法接受的是期待结果，于是一句expectation可以写成：

  it("and has a positive case", function() {
    expect(true).toBe(true);
  });

toBe()就是Matcher方法。Matcher方法实现了期待值和实际结果值的比较，并把布尔型的比较结果报告给jasmine，然后jasmine在来是一个测试 pass或是fail
...
使用fail()方法来让一个spec失败。fail()的参数可以是一个failure message或是Error object
...
jasmine提供的是全局beforeEach(), afterEach(), beforeAll()和afterAll()方法

beforeEach()和afterEach()会在每个spec前后执行
...
beforeAll()和afterAll()会在所有spec前后执行。不过，我们要小心使用这两个方法因为他们不会再spec之间充值状态。
...
this 关键字
我们可以通过在describe里面设置全局变量来传递此变量到it()中。而在beforeEach, it和afterEach中我们可以通过this关键字来分享变量。但是对下一个beforeEach, it和afterEach this关键字都会被重置为empty file
...
调用beforeAll, afterAll, beforeEach, afterEach和it时，可以传入单个参数，这个参数被调用的时候就代表着一步操作的结束
